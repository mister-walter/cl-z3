...
`python scripts/gen_z3_grovel.py <z3-path> -o <output-file>`
...
`python scripts/gen_z3_grovel.py <z3-path> -o src/ffi/z3-grovel.lisp -f`
...
def get_typedef_enum(ty, defs):
    '''Given a type, if that type is an enum typedef, get the
    underlying enum type from the given defs. Otherwise, return None.'''
    if ty.is_fund_type() and ty.type_spec.startswith("enum "):
        enum_wrapper = defs['types'][ty.type_spec]
        assert(enum_wrapper.type_spec == "enum")
        enum_ty = defs['enums'][enum_wrapper.declarators[0]]
        return enum_ty
    return None

def gather_enums(defs):
    enums = {}
    for name, ty in defs['types'].items():
        enum = get_typedef_enum(ty, defs)
        if enum is not None:
            enums[name] = list(enum.keys())
    return enums

def output_enum_value(val, outfile):
    outfile.write(f'((:{lispify_c_name(val)} "{val}"))\n')

def process_enums(enums, outfile):
    for name, values in enums.items():
        outfile.write(f'(cenum ({lispify_c_name(name)})\n')
        for value in values:
            outfile.write('       ')
...
def process_opaque_pointers(defs, outfile):
    ...
    for name in defs['variables'].keys():
        outfile.write(f'(ctype {name} "{name}")\n')
...
def gen_grovel_file(defs, outfile):
    grovel_header = [
        ...
        ';; See scripts/gen_z3_grovel.py for more information',
